...
# read in PR_reads and add

PR_reads<-read_csv("reads/pr_reads.csv")
...
#  facet_wrap(subsample_perc~data_type,scales = "free_x")+
...
top_PR$f_score<-gsub("NaN",0,top_PR$f_score)
top_PR$f_score<-as.numeric(top_PR$f_score)

top_PR%>%
  #filter(assembler!="SH" & assembler!="MH")%>%
  ggplot(aes(x=subsample_perc,y=f_score,colour = tool))+
  ...
  labs(x="Percentage of Reads",y="F1 - Score")+
