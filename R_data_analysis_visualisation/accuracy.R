    ...
    for (i  in ID) {
      for (j  in QC) {
        ...
        hit_list<-hit_file%>%
          ...
          separate_wider_delim(sseqid,"|",names = c("gene_name","ARO","family"))%>%
          pull(ARO)
        for (n in 1:nrow(classification_file)) {
          classification_file[n,column] <- length(grep(classification_file[n,"ARO_accession"],hit_list))
        }
...
#write_csv(PR_all,"PR_all_data_binary.csv")
...
PR_all%>%
  ...
  labs(x="Recall",y="Precision",title = "Minigraph AMR gene level Precision vs Recall" )
...
PR_all%>%
  ...
  geom_text(vjust=1.5, size = 2,check_overlap = TRUE)+
  facet_wrap(~data_type,scales = "free_x")+
  labs(x="Recall",y="Precision",title = "Bifrost AMR gene level Precision vs Recall" )
...
PR_all%>%
  ...
  geom_text(vjust=1.5, size = 2,check_overlap = TRUE)+
  facet_wrap(~data_type,scales = "free_x")+
  labs(x="Recall",y="Precision",title = "Bandage AMR gene level Precision vs Recall" )
...
PR_all%>%
  ...
  geom_text(vjust=1.5, size = 2,check_overlap = TRUE)+
  facet_wrap(~data_type,scales = "free_x")+
  labs(x="Recall",y="Precision",title = "Graphaligner AMR gene level Precision vs Recall" )
...
PR_all%>%
  ...
  geom_text(vjust=1.5, size = 2,check_overlap = TRUE)+
  facet_wrap(~data_type,scales = "free_x")+
  labs(x="Recall",y="Precision",title = "SPAligner AMR gene level Precision vs Recall" )
...
PR_all%>%
  ...
  geom_text(vjust=1.5, size = 2,check_overlap = TRUE)+
  facet_wrap(~data_type,scales = "free_x")+
  labs(x="Recall",y="Precision",title = "Pathracer AMR gene level Precision vs Recall" )
...
PR_all%>%
  ...
  geom_text(vjust=1.5, size = 2,check_overlap = TRUE)+
  facet_wrap(~data_type,scales = "free_x")+
  labs(x="Recall",y="Precision",title = "Contigs AMR gene level Precision vs Recall" )
...
# HC only
top_PR%>%
  filter(data_type=="HC" & tool!="CON" & tool!="READS")%>%
  ggplot(aes(x=recall,y=precision,colour = tool))+
  geom_point(aes(shape=read_type),size=8)+
  ...
  labs(x= "Recall",y="Precision",shape="Read Type")+
...
# without the contigs and reads
top_PR%>%
  filter(tool!="CON" & tool!="READS")%>%
  ggplot(aes(x=recall,y=precision,colour = tool))+
  geom_point(aes(shape=read_type),size=8)+
  facet_grid(read_type~data_type,scales = "free_x")+
  scale_color_brewer(type = "qual",palette = "Dark2",name = "Tool",labels=c("Bandage","Bifrost","GraphAligner","Minigraph","Pathracer","SPAligner","Contigs","Reads"))+
...
top_PR%>%
  filter(tool!="READS")%>%
  ggplot(aes(x=recall,y=precision,colour = tool))+
  geom_point(aes(shape=assembler),size=3)+
  facet_grid(read_type~data_type,scales = "free_x")+
  scale_color_brewer(type = "qual",palette = "Dark2",name = "Tool",labels=c("Bandage","Bifrost","GraphAligner","Minigraph","Pathracer","SPAligner","Contigs","Reads"))+
...
top_PR%>%
  filter(tool!="READS")%>%
  ggplot(aes(x=recall,y=precision,colour = tool,label=subsample_perc))+
  geom_point(aes(shape=read_type),size=0)+
  geom_text()+
  facet_grid(read_type~data_type,scales = "free_x")+
  scale_color_brewer(type = "qual",palette = "Dark2",name = "Tool",labels=c("Bandage","Bifrost","GraphAligner","Minigraph","Pathracer","SPAligner","Contigs","Reads"))+
  labs(x= "Recall",y="Precision",shape="Read Type")+
...
top_PR%>%
  ...
  labs(x= "Recall",y="Precision",shape="Assembler")+
  theme(text = element_text(size = 12))+
  scale_shape_manual(values = c(6,15,17,12))+
  xlim(0,1)


##Subsample lines
top_PR$subsample_perc<-as.numeric(top_PR$subsample_perc)

top_PR%>%
  #filter(assembler!="SH" & assembler!="MH")%>%
  ggplot(aes(x=subsample_perc,y=area,colour = tool))+
  geom_point(aes(shape=assembler))+
  geom_line(aes(linetype=assembler))+
  facet_grid(read_type~data_type)+
  scale_color_brewer(type = "qual",palette = "Dark2",name = "Tool",labels=c("Bandage","Bifrost","GraphAligner","Minigraph","Pathracer","SPAligner","Contigs","Reads"))+
  scale_shape_manual(values = c(6,15,17,12))+
  scale_linetype_manual(values = c(1,1,1,1,1))+
  labs(x="Percentage of Simulated Reads",y="Precision and Recall Product")
