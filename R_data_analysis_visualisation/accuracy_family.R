...
cluster_codes$cluster<-paste(cluster_codes$family,cluster_codes$cluster,"clust",sep = "_")
...
for (i in 1:nrow(classifications)) {
  classifications[i,"actual_HC_classification"]<-length(grep(classifications[i,"ARO"], HC_baseline$ARO, fixed = TRUE))
}

classifications$actual_LC_classification <- 0
for (i in 1:nrow(classifications)) {
  classifications[i,"actual_LC_classification"]<-length(grep(classifications[i,"ARO"], LC_baseline$ARO))
}

classifications$actual_RZ_classification <- 0
for (i in 1:nrow(classifications)) {
  classifications[i,"actual_RZ_classification"]<-length(grep(classifications[i,"ARO"], RZ_baseline$ARO))
}  

classifications$actual_ME_classification <- 0
for (i in 1:nrow(classifications)) {
  classifications[i,"actual_ME_classification"]<-length(grep(classifications[i,"ARO"], ME_baseline$ARO))
}  

#group by cluster
classifications<-classifications%>%
  group_by(cluster)%>% 
  summarise(actual_HC_classification = sum(actual_HC_classification),
            actual_LC_classification = sum(actual_LC_classification),
            actual_RZ_classification = sum(actual_RZ_classification),
            actual_ME_classification = sum(actual_ME_classification))
    ...
    for (i  in ID) {
      for (j  in QC) {
        ...
        for (n in 1:nrow(classification_file)) {
          classification_file[n,column] <- length(grep(classification_file[n,"cluster"],hit_list, fixed = TRUE))
        }
      }
    }
